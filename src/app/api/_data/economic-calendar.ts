...
const getCachedEconomicCalendarRange = unstable_cache(
  async (key: EconomicCalendarRangeKey): Promise<EconomicCalendarRangeData> => {
    const endpoint = ECONOMIC_CALENDAR_ENDPOINTS[key];
    const response = await fetch(
      `${ECONOMIC_CALENDAR_API_BASE_URL}/${endpoint}`,
      {
        headers: {
  ...
  ["economic-calendar-range"],
  {
    revalidate: 300,
  },
);

export async function getEconomicCalendarRangeData(
  key: EconomicCalendarRangeKey,
) {
  return getCachedEconomicCalendarRange(key);
}
