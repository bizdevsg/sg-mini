...
function LegalArticle({
  ...
  // Build a table of contents from the rendered headings and give each one a
  // stable, unique id so the sidebar and browser deep-links can target them.
  useEffect(() => {
  ...
  // Drive the gold progress rail off how far through the article the reader has scrolled.
  useEffect(() => {
    const onScroll = () => {
  ...
  return (
    <div className={hasToc ? "lg:grid lg:grid-cols-[220px_minmax(0,1fr)] lg:gap-12" : ""}>
      {hasToc ? (
        <aside className="hidden self-start lg:block">
          <div className="sticky top-28">
            <div className="relative pl-4">
              ...
              <nav className="flex flex-col gap-0.5 text-sm" aria-label={onThisPageLabel}>
                {toc.map((item) => (
                  <a
                    key={item.id}
                    href={`#${item.id}`}
