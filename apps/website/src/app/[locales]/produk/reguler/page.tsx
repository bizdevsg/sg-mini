...
export async function generateMetadata({
  params,
}: ProdukRegulerPageProps): Promise<Metadata> {
  const { locales } = await params;
  assertValidLocale(locales);

  const page = getMessages(locales).produkRegulerPage;

  return {
    title: page.title,
    description: page.description,
    alternates: {
      canonical: `/${locales}/produk/reguler`,
      languages: Object.fromEntries(
        SUPPORTED_LOCALES.map((locale) => [
          getLocaleConfig(locale).lang,
          `/${locale}/produk/reguler`,
        ]),
      ),
    },
  };
...
export default async function ProdukRegulerPage({
  params,
}: ProdukRegulerPageProps) {
  const { locales } = await params;
  assertValidLocale(locales);

  const appMessages = getMessages(locales);
  const page = appMessages.produkRegulerPage;
  const productLabel =
    appMessages.navbar.menuGroups.find((group) =>
      group.items?.some((item) => item.href === "/produk/reguler"),
  ...
  return (
    <main>
      ...
      <div className="relative">
        <div className="absolute w-full h-100 bg-linear-to-b from-black to-transparent" />
        ...
        <ScrollReveal effect="fade-up">
          <div className="mx-auto rounded-[2rem] border border-line bg-[linear-gradient(160deg,rgba(205,161,58,0.18),rgba(10,10,10,0.92)_42%,rgba(10,10,10,0.98))] p-8 text-center sm:p-12">
            <SectionIntro
              ...
              titleClassName="max-w-2xl mx-auto tracking-[-0.03em] sm:text-4xl"
