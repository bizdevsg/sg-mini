      ...
      <div className="mx-auto">
        <div className="grid gap-4 sm:grid-cols-2">
          {items.map((item, index) => (
            <ScrollReveal
              key={item.title}
              effect={index % 3 === 0 ? "zoom-in" : index % 3 === 1 ? "fade-up" : "fade-left"}
              delay={index * 120}
            >
              <article
                className="group relative flex flex-col h-full rounded-[20px] border border-line bg-linear-to-br from-slate-900/40 to-slate-900/20 p-6 backdrop-blur-sm transition-all duration-300 hover:border-yellow-500/50 hover:shadow-lg hover:shadow-yellow-500/10 sm:p-8"
              >
