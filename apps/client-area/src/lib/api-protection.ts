...
// Allows the sibling website app (same registrable domain, e.g.
// sg-berjangka.com vs client.sg-berjangka.com, or same localhost host on a
// different dev port) to call a deliberately cross-app endpoint, while still
// rejecting cross-site/spoofed requests. sec-fetch-site is browser-computed
// and unspoofable, unlike Origin/Referer — see isTrustedSameOriginBrowserRequest.
export function resolveTrustedBrowserOrigin(request: Request) {
  const secFetchSite = request.headers.get("sec-fetch-site")?.toLowerCase();
  const origin = request.headers.get("origin");

  if (!origin || (secFetchSite !== "same-origin" && secFetchSite !== "same-site")) {
    return null;
  }
